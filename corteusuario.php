...
if($_POST['ajax']==1){
	...
	$res = mysql_query("SELECT usuario,MIN(CONCAT(fecha,' ',hora)),MAX(CONCAT(fecha,' ',hora)),SUM(monto) FROM abono_conteorapido WHERE estatus!='C' AND {$filtrofecha} BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'".$filtro." GROUP BY usuario");
	while($row = mysql_fetch_array($res)){
		$array_usuarios[$row['usuario']] = $array_usuario[$row['usuario']];
		$array_datos[$row['usuario']]['fecha_ini_conteorapido'] = $row[1];
		$array_datos[$row['usuario']]['fecha_fin_conteorapido'] = $row[2];
		$array_datos[$row['usuario']]['conteorapido'] = $row[3];
	}
	...
	foreach($array_usuarios as $k=>$v){
		...
		if($array_datos[$k]['conteorapido'] > 0){
			rowb();
			echo '<td>Conteo Rapido</td><td align="right">'.number_format($array_datos[$k]['conteorapido'],2).'</td>';
			echo '<td align="center">'.$array_datos[$k]['fecha_ini_conteorapido'].'</td>';
			echo '<td align="center">'.$array_datos[$k]['fecha_fin_conteorapido'].'</td></tr>';
		}
		...
		<th align="right">'.number_format($array_datos[$k]['abono']+$array_datos[$k]['entradas']+$array_datos[$k]['mutualidad']+$array_datos[$k]['desglose']+$array_datos[$k]['taquilla']+$array_datos[$k]['taquillamovil']+$array_datos[$k]['abonotaquillamovil']+$array_datos[$k]['conteorapido'],2).'</th><th colspan="2">&nbsp;</th></tr></table>';
	}
	exit();
}
top($_SESSION);
...
if ($_POST['cmd']<1) {
	...
	$res = mysql_query("SELECT usuario FROM abono_conteorapido WHERE estatus!='C'  GROUP BY usuario");
	while($row = mysql_fetch_array($res)){
		if(array_key_exists($row['usuario'], $array_usuario))
			$array_usuarios[$row['usuario']] = $array_usuario[$row['usuario']];
	}
