...
$array_motivo=array();
$res=mysql_db_query($base,"SELECT * FROM ".$pre."motivos_condonacion ORDER BY nombre");
while($row=mysql_fetch_array($res)){
	$array_motivo[$row['cve']]=$row['nombre'];
}

$array_empresas=array();
$res=mysql_db_query($base,"SELECT * FROM ".$pre."empresas ORDER BY nombre");
while($row=mysql_fetch_array($res)){
	$array_empresas[$row['cve']]=$row['nombre'];
}

$rsUnidad=mysql_db_query($base,"SELECT * FROM ".$pre."parque");
...
$res=mysql_db_query($base,"SELECT * FROM ".$pre."conductores");
while($row=mysql_fetch_array($res)){
...
if($_POST['cmd']==200){
	...
	class PDF1 extends FPDF{
		...
		function Header(){
			...
			//Título
			$this->MultiCell(0,10,'Lista de Folios de Condonacion a Unidades del dia: '.$fecha1.' al dia '.$fecha2,0,'C');
			//Salto de línea
			$this->Ln(20);
		}
	...
	$select= " SELECT a.* FROM ".$pre."condonacion as a INNER JOIN ".$pre."parque as b ON (b.cve=a.unidad";
	if(trim($_POST['no_eco'])!="")$select.=" AND b.no_eco='".strtoupper($_POST['no_eco'])."'";
	$select.=") INNER JOIN ".$pre."conductores as c ON (c.cve=a.conductor";
	...
	$pdf->Cell(20,5,'Empresa',1,0,'C');
	$pdf->Cell(20,5,'Unidad',1,0,'C');
	$pdf->Cell(50,5,'Conductor',1,0,'L');
	...
	while ($row=mysql_fetch_array($res)){	
		...
		$pdf->Cell(20,5,$array_empresas[$row['empresa']],1,0,'L');
		$pdf->Cell(20,5,$array_unidad[$row['unidad']],1,0,'L');
		$pdf->Cell(50,5,$array_nomconductor[$row['conductor']],1,0,'L');
...
if($_POST['ajax']==1){
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		echo '<th><a href="#" onClick="buscarRegistros(0,'.$tipoorden0.')">Folio</a></th><th>Fecha</th><th>Fecha Cuenta</th><th>Tarjeta</th><th>Empresa</th>
		...
		$res1=mysql_db_query($base,"SELECT a.usuario FROM ".$pre."condonacion as a WHERE 1 $filtro GROUP BY a.usuario ORDER BY a.usuario");
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			$res1=mysql_db_query($base,"SELECT a.*,IFNULL(c.cve,0) as recaudado FROM ".$pre."parque_tarjetas a LEFT JOIN ".$pre."parque_abono as c ON (c.tarjeta=a.cve AND c.estatus!='C') WHERE a.cve='".$Abono['cvetar']."'");
			...
			if($Abono['estatus']=='C'){
				...
				echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'condonacion.php\',\'\',\'201\','.$Abono['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
			...
			echo '<td align="center">'.$row1['folio'].'</td>';
			echo '<td align="center">'.$array_empresas[$Abono['empresa']].'</td>';
...
if($_POST['ajax']==2){
	mysql_db_query($base,"UPDATE ".$pre."condonacion SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['usuario']."',obscan='".$_POST['obs']."' WHERE cve='".$_POST['abono']."'") or die(mysql_error());
	exit();
}


if($_POST['ajax']==7){
	$res=mysql_db_query($base,"SELECT * FROM ".$pre."parque_tarjetas WHERE cve='".$_POST['ftarjeta']."'");
	if($row=mysql_fetch_array($res)){
		if($row['estatus']=='C'){
			echo "-3|";
		}
		else{
			$res1=mysql_db_query($base,"SELECT cve FROM ".$pre."parque_abono WHERE tarjeta='".$row['cve']."' AND estatus!='C'");
			if(mysql_num_rows($res1)>0){
				echo "-2|";
			}
			else{
				$res1=mysql_db_query($base,"SELECT SUM(monto) FROM ".$pre."condonacion WHERE cvetar='".$row['cve']."' AND estatus!='C'");
				$row1=mysql_fetch_array($res1);
				echo $row['cve'].'|'.$row['empresa'].'|'.$array_empresas[$row['empresa']].'|'.$row['fecha_cuenta'].'|'.$row['unidad'].'|'.$array_unidad[$row['unidad']].'|'.$row['conductor'].'|'.$array_nomconductor[$row['conductor']].'|'.round($row['cuenta']-$row1[0],2);
			}
		}
	}
	else{
...
if($_POST['cmd']==201){
	$res = mysql_db_query($base,"SELECT * FROM ".$pre."condonacion WHERE cve='".$_POST['reg']."'");
	...
	$varimp.="Empresa: ".$array_empresas[$row['empresa']]."|";
	...
	if($row['devolucion']>0){
		$varimp="AEET Devolucion||Folio: ".$row['folio']."||Ope:".$array_conductor[$row['conductor']].'||Unidad:'.$array_unidad[$row['unidad']].'||Monto: '.number_format($row['devolucion'],2).'||Usuario: '.$array_usuario[$row['usuario']].'||';
		$impresion.='<iframe src="http://localhost/impresiongenerallogo.php?textoimp='.$varimp.'" width=200 height=200></iframe>';
	}
	$_POST['cmd']=0;
}
...
if($_POST['cmd']==2){
		mysql_db_query($base,"INSERT ".$pre."condonacion SET cvetar='".$_POST['cvetar']."',fecha_cuenta='".$_POST['fecha_cuenta']."',
		fecha='".fechaLocal()."',hora='".horaLocal()."',empresa='".$_POST['empresa']."',motivo='".$_POST['motivo']."',
		...
		$res=mysql_db_query($base,"SELECT * FROM ".$pre."condonacion WHERE cve='$abono'");
		$row=mysql_fetch_array($res);
...
if($_POST['cmd']==1){
	...
	if(nivelUsuario()>1){
		...
		if(document.forma.tarjeta.value==\'\'){
			...
			alert(\'Necesita ingresar el monto de la condonacion\');
			$(\'#panel\').hide();
		}
			...
			atcr(\'condonacion.php\',\'\',2,\'0\');
		}
		"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
	}
	echo '<td><a href="#" onclick="$(\'#panel\').show();atcr(\'condonacion.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>';
	echo '</tr></table>';
	echo '<br>';
	echo '<table>';
	...
	echo '</td></tr>';
	echo '<tr><td>Empresa</td><td colspan="2"><input type="text" name="nomempresa" id="nomempresa" size="10" value="" class="readOnly" readOnly></td></tr>';
	...
	echo '<script>
			...
			function traeTarjeta(){
				...
				if (objeto1.readyState != 0) {
					...
					objeto1.open("POST","condonacion.php",true);
					...
					objeto1.onreadystatechange = function(){
						if (objeto1.readyState==4){
							...
							if(datos[0]=="-1"){
								...
								document.forma.empresa.value=datos[1];
								document.forma.nomempresa.value=datos[2];
								document.forma.fecha_cuenta.value=datos[3];
								document.forma.unidad.value=datos[4];
								document.forma.num_eco.value=datos[5];
								document.forma.conductor.value=datos[6];
								document.forma.nomconductor.value=datos[7];
								document.forma.saldo.value=datos[8];
							}
						}
					}
				}
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onclick="atcr(\'condonacion.php\',\'\',1,0);"><img src="images/nuevo.gif" border="0">&nbsp;&nbsp;Condonar</a>&nbsp;&nbsp;</td>';
		echo '
				<!--<td><a href="#" onClick="atcr(\'condonacion.php\',\'_blank\',\'200\',\'\')"><img src="images/b_print.png" border="0" title="Imprimir">&nbsp;Imprimir</a></td>-->
			  </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros(orden,tipoorden)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","condonacion.php",true);
	...
	function cancelarAbono(numabono){
	  if(confirm("¿Esta seguro de cancelar la condonacion?")){
