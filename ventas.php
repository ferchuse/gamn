...
while($row=mysql_fetch_array($res)){
	$array_clientes[$row['cve']]=$row['no_eco'];
}
$option_producto='';
...
$rsUsuario=mysql_db_query($base,"SELECT porcentaje FROM tasa_venta");
$porcentaje=mysql_fetch_array($rsUsuario);
$tasa_venta=(number_format($porcentaje['porcentaje'],2))/(100);
if($_POST['cmd']==101){
	if($_POST['mostrar'] == 1){
		$select="SELECT a.cve,a.fecha,a.hora,a.comprador,a.obs,a.estatus,a.usuario,a.usucan,a.fechacan,b.producto,b.cantidad, a.fecha_creacion
...
if($_POST['ajax']==1){
		$select="SELECT a.cve,a.fecha,a.hora,a.cliente,a.obs,a.estatus,a.usuario,a.usucan,a.fechacan, a.total, a.fecha_creacion
...
if($_GET['ajax']==3){
	...
	while($row = mysql_fetch_array($res)){
		$resultado[] = array(
			...
			'precio_venta' => round($row['costo']+($row['costo']*$tasa_venta),2)
		);
	}
...
if ($_POST['cmd']==1) {
	...
	echo '<table>';
	echo '<tr><th align="left" colspan="2">'.$Encabezado.'</th><td>&nbsp;</td></tr>';
