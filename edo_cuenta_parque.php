...
if($_POST['cmd']==100){
	...
	$pdf->Cell(0,10,"Listado de Estado de Cuenta de Unidades del ".$_POST['fecha_ini']." al ".$_POST['fecha_fin'],0,0,"L");
    $pdf->Ln();
	$pdf->Ln();
	$pdf->SetFont('Arial','',9);
...
if($_POST['cmd']==102){
	...
	for($i=0;$i<count($_POST['unidades']);$i++){
		...
		$pdf->Cell(0,10,"Estado de Cuenta de la Unidad ".$row['no_eco']." Propietario ".$array_propietario[$row['propietario']]." ".fechaLocal()." ".horaLocal(),0,0,"L");
		$pdf->Ln();
		$pdf->Ln();
		$pdf->SetFont('Arial','',9);
		...
		$pdf->Ln();
		$saldo = saldo_unidad($row['cve'],1,0,$_POST['fecha_ini'],"");
		$x=$abono=$cargo=0;
		...
		while($fecha<=$_POST['fecha_fin']){
			$res=mysql_db_query($base,"SELECT * FROM cargos_parque WHERE unidad='".$_POST['unidades'][$i]."' AND fecha>='2016-11-30' AND fecha='$fecha'");
			while($row=mysql_fetch_array($res)){
				$cargo+=abs($row['monto']);
				$saldo-=abs($row['monto']);
				$pdf->Cell(18,6,$fecha,1,0,'C');
				$pdf->Cell(65,6,$array_cargos_unidades[$row['motivo']],1,0,'C');
				$pdf->Cell(20,6,number_format(abs($row['monto']),2),1,0,'C');
				$pdf->Cell(20,6," ",1,0,'C');
				$pdf->Cell(20,6,number_format($saldo,2),1,0,'C');
				$pdf->Cell(55,6," ",1,0,'C');
				$pdf->Ln();
			}
			$res=mysql_db_query($base,"SELECT * FROM cargos_variables_unidades WHERE unidad='".$_POST['unidades'][$i]."' AND fecha_ini>='2016-11-30' AND fecha_ini='$fecha' AND sta!='C'");
			while($row=mysql_fetch_array($res)){
				$cargo+=abs($row['total']);
				$saldo-=abs($row['total']);
				$pdf->Cell(18,6,$fecha,1,0,'C');
				$pdf->Cell(65,6,$array_cargosv_unidades[$row['motivo']],1,0,'C');
				$pdf->Cell(20,6,number_format(abs($row['total']),2),1,0,'C');
				$pdf->Cell(20,6," ",1,0,'C');
				$pdf->Cell(20,6,number_format($saldo,2),1,0,'C');
				$pdf->Cell(55,6," ",1,0,'C');
				$pdf->Ln();
			}
			$res=mysql_db_query($base,"SELECT a.cve,b.monto,a.referencia,a.concepto FROM traspaso as a INNER JOIN traspasomov as b ON (b.traspaso=a.cve AND b.unidad='".$_POST['unidades'][$i]."') WHERE a.estatus!='C' AND a.fechaapl>='2016-11-30' AND a.fechaapl='$fecha'");
			while($row=mysql_fetch_array($res)){
				$cargo+=$row['monto'];
				$saldo-=$row['monto'];
				$pdf->Cell(18,6,$fecha,1,0,'C');
				$pdf->Cell(65,6,"Retiro a Cuenta de Liquidacion # ".$row['cve'].", Ref: ".$row['referencia'],1,0,'C');
			...
			$res=mysql_db_query($base,"SELECT * FROM parque_abono WHERE unidad='".$_POST['unidades'][$i]."' AND estatus!='C' AND fecha>='2016-11-30' AND fecha='$fecha'");
			while($row=mysql_fetch_array($res)){
				$abono+=$row['monto'];
				$saldo+=$row['monto'];
				$pdf->Cell(18,6,$fecha,1,0,'C');
				$pdf->Cell(65,6,"Abono # ".$row['cve'],1,0,'C');
				$pdf->Cell(20,6,number_format(0,2),1,0,'C');
				$pdf->Cell(20,6,number_format($row['monto'],2),1,0,'C');
				$pdf->Cell(20,6,number_format($saldo,2),1,0,'C');
				$pdf->Cell(55,6,$row['concepto'],1,0,'C');
			...
			$res=mysql_db_query($base,"SELECT * FROM recibos_entrada WHERE unidad='".$_POST['unidades'][$i]."' AND fecha>='2016-11-30' AND estatus!='C' AND fecha='$fecha'");
			while($row=mysql_fetch_array($res)){
				$abono+=$row['monto'];
				$saldo+=$row['monto'];
				$pdf->Cell(18,6,$fecha,1,0,'C');
				$pdf->Cell(65,6,"Recibo de Entrada # ".$row['cve'],1,0,'C');
				$pdf->Cell(20,6,number_format(0,2),1,0,'C');
				$pdf->Cell(20,6,number_format($row['monto'],2),1,0,'C');
				$pdf->Cell(20,6,number_format($saldo,2),1,0,'C');
				$pdf->Cell(55,6,$row['concepto'],1,0,'C');
...
if($_POST['cmd']==101){
	...
	echo '</table>';
	echo '<br>';
	echo '<h1>Estado de Cuentaa de la Unidad '.$row['no_eco'].' Propietario '.$array_propietario[$row['propietario']].'</h1>'.fechaLocal().' '.horaLocal().'</br>';
	...
	echo '</tr>';
	while($fecha<=$_POST['fecha_fin']){
		$res=mysql_db_query($base,"SELECT * FROM cargos_parque WHERE unidad='".$_POST['reg']."' AND fecha>='2016-11-30' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			$cargo+=abs($row['monto']);
			$saldo-=abs($row['monto']);
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>'.$array_cargos_unidades[$row['motivo']].'&nbsp;</td>';
		...
		$res=mysql_db_query($base,"SELECT * FROM cargos_variables_unidades WHERE unidad='".$_POST['reg']."' AND fecha_ini>='2016-11-30' AND fecha_ini='$fecha' AND sta!='C'");
		while($row=mysql_fetch_array($res)){
			$cargo+=abs($row['total']);
			$saldo-=abs($row['total']);
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>'.$array_cargosv_unidades[$row['motivo']].'&nbsp;</td>';
			echo '<td align="right">'.number_format(abs($row['total']),2).'</td>';
			echo '<td align="right">&nbsp;</td>';
			echo '<td align="right">'.number_format($saldo,2).'</td>';
			echo '<td align="left">&nbsp;</td>';
			echo '</tr>';
		}
		$res=mysql_db_query($base,"SELECT a.cve,b.monto,a.referencia,a.concepto FROM traspaso as a INNER JOIN traspasomov as b ON (b.traspaso=a.cve AND b.unidad='".$_POST['reg']."') WHERE a.estatus!='C' AND a.fechaapl>='2016-11-30' AND a.fechaapl='$fecha'");
		while($row=mysql_fetch_array($res)){
			$cargo+=$row['monto'];
			$saldo-=$row['monto'];
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>&nbsp;Retiro a Cuenta de Liquidacion # '.$row['cve'].', Ref: '.$row['referencia'].'</td>';
		...
		$res=mysql_db_query($base,"SELECT * FROM parque_abono WHERE unidad='".$_POST['reg']."' AND fecha>='2016-11-30' AND estatus!='C' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			$abono+=$row['monto'];
			$saldo+=$row['monto'];
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>&nbsp;Abono # '.$row['cve'].'</td>';
			echo '<td align="right">'.number_format(0,2).'</td>';
			echo '<td align="right">'.number_format($row['monto'],2).'</td>';
			echo '<td align="right">'.number_format($saldo,2).'</td>';
			echo '<td align=left>'.$row['concepto'].'&nbsp;</td>';
		...
		$res=mysql_db_query($base,"SELECT * FROM recibos_entrada WHERE unidad='".$_POST['reg']."' AND fecha>='2016-11-30' AND estatus!='C' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			$abono+=$row['monto'];
			$saldo+=$row['monto'];
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>&nbsp;Recibo de Entrada # '.$row['cve'].'</td>';
			echo '<td align="right">'.number_format(0,2).'</td>';
			echo '<td align="right">'.number_format($row['monto'],2).'</td>';
			echo '<td align="right">'.number_format($saldo,2).'</td>';
			echo '<td align=left>'.$row['concepto'].'&nbsp;</td>';
...
if($_POST['ajax']==1){
	...
	if ($_POST['unidad']!="") { $select.=" AND no_eco='".$_POST['unidad']."'"; }
	if ($_POST['estatus']!="all") { $select.=" AND estatus='".$_POST['estatus']."'"; }
	if ($_POST['propietario']!="") { $select.=" AND propietario='".$_POST['propietario']."'"; }
	$select.=" ORDER BY no_eco";
	$res=mysql_db_query($base,$select);
	if(mysql_num_rows($rsconductor)>0) {
		echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
		echo '<tr><td bgcolor="#E9F2F8" colspan="12">'.mysql_num_rows($rsconductor).' Registro(s)</td></tr>';
		echo '<tr bgcolor="#E9F2F8">';
		echo '<th><input type="checkbox" onClick="if(this.checked) $(\'.chks\').attr(\'checked\',\'checked\'); else $(\'.chks\').removeAttr(\'checked\');"></th><th>No Economico</th><th>Estatus</th><th>Propietario</th><th>Saldo Anterior</th><th>Traspaso de Utilidad</th><th>Cargos administrativos</th><th>Cargos Totales</th><th>Recaudacion</th><th>Recibos de Entrada</th><th>Abonos Totales</th><th>Saldo</th>';
		echo '</tr>';
		$i=0;
		$x=0;
		$totales=array();
		...
		echo '<td align="center">'.$array_propietario[$row['propietario']].'</td>';
			$saldo_anterior = saldo_unidad($row['cve'],1,0,$_POST['fecha_ini'],"");
			$cargo = saldo_unidad($row['cve'],2,1,$_POST['fecha_ini'],$_POST['fecha_fin'], true);
			$abono = saldo_unidad($row['cve'],2,2,$_POST['fecha_ini'],$_POST['fecha_fin'], true);
			echo '<td align="right">'.number_format($saldo_anterior,2).'</td>';
			echo '<td align="right">'.number_format($cargo['traspaso'],2).'</td>';
			echo '<td align="right">'.number_format($cargo['cargo_admon'],2).'</td>';
			echo '<td align="right">'.number_format($cargo['cargo'],2).'</td>';
			echo '<td align="right">'.number_format($abono['recaudacion'],2).'</td>';
			echo '<td align="right">'.number_format($abono['entradas'],2).'</td>';
			echo '<td align="right">'.number_format($abono['abono'],2).'</td>';
			echo '<td align="right"><a href="#" onClick="atcr(\'edo_cuenta_parque.php\',\'\',1,'.$row['cve'].')">'.number_format($saldo_anterior+$abono['abono']-$cargo['cargo'],2).'</a></td>';
			echo '</tr>';
			$totales[0]+=$saldo_anterior;
			$totales[1]+=$cargo['traspaso'];
			$totales[2]+=$cargo['cargo_admon'];
			$totales[3]+=$cargo['cargo'];
			$totales[4]+=$abono['recaudacion'];
			$totales[5]+=$abono['entradas'];
			$totales[6]+=$abono['abono'];
			$totales[7]+=$saldo_anterior+$abono['abono']-$cargo['cargo'];
			$i++;
		}
		echo '	
			<tr>
...
if($_POST['ajax']==2){
	...
	echo '<tr><td class="tableEnc">Estado de Cuenta de la Unidad '.$row['no_eco'].' Propietario '.$array_propietario[$row['propietario']].'</td></tr>';
	echo '</table>';
	echo '<br>';
	echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
	echo '<tr bgcolor="#E9F2F8">';
	...
	echo '</tr>';
	while($fecha<=$_POST['fecha_fin']){
		$res=mysql_db_query($base,"SELECT * FROM cargos_parque WHERE unidad='".$_POST['unidad']."' AND fecha>='2016-11-30' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			$cargo+=abs($row['monto']);
			$saldo-=abs($row['monto']);
			rowb();
		...
		$res=mysql_db_query($base,"SELECT * FROM cargos_variables_unidades WHERE unidad='".$_POST['unidad']."' AND fecha_ini>='2016-11-30' AND fecha_ini='$fecha' AND sta!='C'");
		while($row=mysql_fetch_array($res)){
			$cargo+=abs($row['total']);
			$saldo-=abs($row['total']);
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>'.$array_cargosv_unidades[$row['motivo']].'&nbsp;#'.$row['cve'].'</td>';
			echo '<td align="right">'.number_format(abs($row['total']),2).'</td>';
			echo '<td align="right">&nbsp;</td>';
			echo '<td align="right">'.number_format($saldo,2).'</td>';
			echo '<td align="left">&nbsp;</td>';
		...
		$res=mysql_db_query($base,"SELECT a.cve,b.monto,a.referencia,a.concepto FROM traspaso as a INNER JOIN traspasomov as b ON (b.traspaso=a.cve AND b.unidad='".$_POST['unidad']."') WHERE a.estatus!='C' AND a.fechaapl>='2016-11-30' AND a.fechaapl='$fecha'");
		while($row=mysql_fetch_array($res)){
			$cargo+=$row['monto'];
			$saldo-=$row['monto'];
			rowb();
		...
		$res=mysql_db_query($base,"SELECT * FROM parque_abono WHERE unidad='".$_POST['unidad']."' AND estatus!='C' AND fecha>='2016-11-30' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			$abono+=$row['monto'];
			$saldo+=$row['monto'];
			rowb();
		...
		$res=mysql_db_query($base,"SELECT * FROM recibos_entrada WHERE unidad='".$_POST['unidad']."' AND fecha>='2016-11-30' AND estatus!='C' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			$abono+=$row['monto'];
			$saldo+=$row['monto'];
			rowb();
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>&nbsp;Recibo Entrada # '.$row['cve'].'</td>';
			echo '<td align="right">'.number_format(0,2).'</td>';
			echo '<td align="right">'.number_format($row['monto'],2).'</td>';
			echo '<td align="right">'.number_format($saldo,2).'</td>';
			echo '<td align=left>'.$row['concepto'].'&nbsp;</td>';
...
if($_POST['cmd']==1){
	...
	echo '<tr>
			...
			<td><a href="#" onclick="atcr(\'edo_cuenta_parque.php\',\'_blank\',101,'.$_POST['reg'].');"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a></td>
			<td><a href="#" onclick="atcr(\'edo_cuenta_parque.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>
		</tr>';
	echo '</table>';
	echo '<table>';
...
if($_POST['cmd']<1){
	/*** PAGINA PRINCIPAL **************************************************/
		...
		echo '<tr>
				...
				<td><a href="#" onclick="atcr(\'edo_cuenta_parque.php\',\'_blank\',100,0);"><img src="images/b_print.png" border="0"></a>&nbsp;&nbsp;Imprimir</td>
				<td><a href="#" onclick="atcr(\'edo_cuenta_parque.php\',\'_blank\',102,0);"><img src="images/b_print.png" border="0"></a>&nbsp;&nbsp;Imprimir Detalle</td>
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","edo_cuenta_parque.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&unidad="+document.getElementById("unidad").value+"&estatus="+document.getElementById("estatus").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&propietario="+document.getElementById("propietario").value);
			objeto.onreadystatechange = function()
			{
				if (objeto.readyState==4)
