...
if($_POST['ajax']==1){
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			$array_total[4]+=$Abono['monto_tijera']*$fac;
			$array_total[5]+=$Abono['efectivo']*$fac;
			$array_total[6]+=$Abono['devolucion']*$fac;
			$array_total[7]+=$Abono['monto']*$fac;
		}
		$col=6;
...
if($_POST['cmd']==2){
	...
	if(mysql_num_rows($res)==0){
		...
		if($_POST['cverec']>0){
			...
			cant_boletos='".$_POST['cant_boletos']."',monto_boletos='".$_POST['monto_boletos']."',
			efectivo='".$_POST['efectivo']."',devolucion='".$_POST['devolucion']."',
			monto_tijera='".$_POST['monto_tijera']."',cant_tijera='".$_POST['cant_tijera']."'") or die(mysql_error()."1");
			$abono=mysql_insert_id();
		}
		...
		if($_POST['devolucion'] > 0){
			mysql_query("INSERT devolucion_unidades SET fecha=CURDATE(),hora=CURTIME(),recaudacion='$abono',
				...
				monto='".$_POST['devolucion']."',estatus='A',usuario='".$_POST['cveusuario']."'");
		}
...
if($_POST['cmd']==1){
	...
	echo '<script>
			...
			function calcular(){
				var total = 0;
				total = document.forma.monto_boletos.value/1 + document.forma.monto_tijera.value/1 + document.forma.efectivo.value/1 - document.forma.tcuenta.value/1;
				if(total>0)
					document.forma.devolucion.value = total.toFixed(2);
				else
					document.forma.devolucion.value = 0;
				total = 0;
				total = document.forma.monto_boletos.value/1 + document.forma.monto_tijera.value/1 + document.forma.efectivo.value/1 - document.forma.devolucion.value/1;
