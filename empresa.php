...
if($_POST['ajax']==1) {
		...
		$select= " SELECT * FROM ".$pre."empresas WHERE 1 ";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			while($row=mysql_fetch_array($res)) {
				rowb();
				echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'empresa.php\',\'\',\'1\','.$row['cve'].')"><img src="images/modificar.gif" border="0" title="Editar '.$row['nombre'].'"></a></td>';
...
if ($_POST['cmd']==1) {
	...
	$select=" SELECT * FROM ".$pre."empresas WHERE cve='".$_POST['reg']."' ";
	$res=mysql_db_query($base,$select);
	...
	echo '
		<tr>';
		if(nivelUsuario()>1)
			echo '<td><a href="#" onClick="if(document.forma.nombre.value==\'\'){alert(\'Necesita introducir el nombre\');}else{atcr(\'empresa.php\',\'\',\'2\',\''.$row['cve'].'\');}"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'empresa.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
		</tr>';
	echo '</table>';
	echo '<br>';
	...
	//Formulario 
	echo '<table>';
	echo '<tr><td class="tableEnc">Edicion de empresas</td></tr>';
	echo '</table>';
	$bloqueado='';
	$class='textField';
	echo '<table>';
...
if ($_POST['cmd']<1) {
	...
	echo '<tr>
			...
			<td><a href="#" onClick="atcr(\'empresa.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			</tr>';
	echo '</table>';
	echo '<br>';
	...
	<Script language="javascript">
		...
		function buscarRegistros()
		{
			...
			if (objeto.readyState != 0) {
				...
				objeto.open("POST","empresa.php",true);
