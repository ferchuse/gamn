...
if($_POST['ajax']==1){
	$filtro="";
	$select= " SELECT a.* FROM vale_tag as a INNER JOIN parque as b ON (b.cve=a.unidad";
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		$res1=mysql_db_query($base,"SELECT a.usuario FROM vale_tag as a WHERE 1 $filtro GROUP BY a.usuario ORDER BY a.usuario");
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			if($Abono['estatus']=='C'){
				...
				echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'vale_tag.php\',\'\',\'201\','.$Abono['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['cve'].'"></a>';
				echo '</td>';
			}
...
if($_POST['cmd']==201){
	$res = mysql_db_query($base,"SELECT * FROM vale_tag WHERE cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	
	$varimp="Vale TAG Nextlalpan|Folio: ".$row['cve']."|";
	...
	$varimp.=chr(29)."h".chr(80).chr(29)."H".chr(2).chr(29)."k".chr(2)."13".sprintf("%010s",(intval($row['cve'])))." |";
...
<Script language="javascript">
	...
	function buscarRegistros(orden,tipoorden)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","vale_tag.php",true);
