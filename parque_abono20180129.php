...
if($_POST['ajax']==1){
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			$array_total[5]+=$Abono['monto_tijera']*$fac;
			$array_total[6]+=$Abono['efectivo']*$fac;
			$array_total[7]+=$Abono['devolucion']*$fac;
			$array_total[8]+=$Abono['monto']*$fac;
		}
		$col=6;
...
if($_POST['ajax']==2){
	if($_POST['clavecancelacion'] == 'Dnbi3z7T'){
		...
		mysql_db_query($base,"UPDATE devolucion_unidades SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['usuario']."' WHERE recaudacion='".$_POST['abono']."'") or die(mysql_error());
		mysql_db_query($base,"UPDATE vale_diesel SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['usuario']."' WHERE abono='".$_POST['abono']."'") or die(mysql_error());
		mysql_db_query($base,"UPDATE vale_tag SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['usuario']."' WHERE abono='".$_POST['abono']."'") or die(mysql_error());
	}
	else{
		echo '1';
	}
	...
	if($row=mysql_fetch_array($res)){
		if($row['estatus']=='C'){
			...
			if(mysql_num_rows($res1)>0){
				...
				$res1=mysql_query("SELECT COUNT(cve), SUM(monto) FROM boletos_taquillamovil WHERE folio_recaudacion=0 AND unidad='".$row['unidad']."'");
				$row1=mysql_fetch_array($res1);
				echo $row1[0].'|'.$row1[1];
			}
		}
...
if($_POST['cmd']==2){
	...
	if(mysql_num_rows($res)==0){
		...
		if($_POST['cverec']>0){
			...
			monto_tijera='".$_POST['monto_tijera']."',cant_tijera='".$_POST['cant_tijera']."',
			monto_taquillamovil='".$_POST['monto_taquillamovil']."',cant_taquillamovil='".$_POST['cant_taquillamovil']."',
			diesel='".$_POST['diesel']."',tag='".$_POST['tag']."',devolucion_efectivo='".$_POST['devolucion_efectivo']."'") or die(mysql_error()."1");
			$abono=mysql_insert_id();
		}
		...
		if($abono>0){
			foreach($_POST['guias'] as $guia){
				...
				mysql_query("UPDATE boletos SET folio_recaudacion='".$abono."',fecha_recaudacion='".fechaLocal()."' WHERE taquilla='".$datos[0]."' AND guia='".$datos[1]."'");
			}
			mysql_query("UPDATE boletos_taquillamovil SET folio_recaudacion='".$abono."',fecha_recaudacion='".fechaLocal()."' WHERE unidad='".$_POST['unidad']."' AND folio_recaudacion='0' ORDER BY cve LIMIT ".intval($_POST['cant_taquillamovil']));
		}
...
if($_POST['cmd']==1){
	...
	echo '<script>
			...
			function calcular(){
				var total = 0;
				total = document.forma.monto_boletos.value/1 + document.forma.monto_taquillamovil.value/1 + document.forma.monto_tijera.value/1 + document.forma.efectivo.value/1 - document.forma.tcuenta.value/1;
				if(total>0)
					document.forma.devolucion.value = total.toFixed(2);
				else
					document.forma.devolucion.value = 0;
				total = 0;
				total = document.forma.monto_boletos.value/1 + document.forma.monto_taquillamovil.value/1 + document.forma.monto_tijera.value/1 + document.forma.efectivo.value/1 - document.forma.devolucion.value/1;
