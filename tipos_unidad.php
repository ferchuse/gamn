...
if($_POST['ajax']==1) {
		...
		$select= " SELECT * FROM ".$pre."tipos_unidad WHERE 1 ";
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
		$select=" SELECT * FROM ".$pre."tipos_unidad WHERE cve='".$_POST['reg']."' ";
		$res=mysql_db_query($base,$select);
		...
		if($row['nombre']!=$_POST['nombre']){
			...
			$update = " UPDATE ".$pre."tipos_unidad 
						SET nombre='".$_POST['nombre']."'
						WHERE cve='".$_POST['reg']."' " ;
			$ejecutar = mysql_db_query($base,$update);			
	} else {
			//Insertar el Registro
			$insert = " INSERT INTO ".$pre."tipos_unidad 
						(nombre)
						VALUES 
						('".$_POST['nombre']."')";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion de Tipos de Unidad</td></tr>';
		echo '</table>';
		$bloqueado='';
		$class='textField';
		if($_POST['reg']>0){
