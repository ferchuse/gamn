...
if($_POST['cmd']==201){
	...
	include("numlet.php");
	$res = mysql_db_query($base,"SELECT * FROM recibos_salidas WHERE cve='".$_POST['reg']."'");
	...
	$pdf->Cell(190,5,$array_beneficiarios[$Salida['beneficiario']],0,0,'C');
	$pdf->Ln();
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(190,5,"Beneficiario",0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
...
if($_POST['cmd']==200){
	...
	class PDF1 extends FPDF{
		...
		function Header(){
			...
			//Título
			$this->MultiCell(0,10,'Lista de Folios de Recibos de Salida del dia: '.$fecha1.' al dia '.$fecha2,0,'C');
			//Salto de línea
			$this->Ln(20);
		}
	...
	$select= " SELECT a.* FROM recibos_salidas as a WHERE a.".$_POST['ffecha'].">='".$_POST['fecha_ini']."' AND a.".$_POST['ffecha']."<='".$_POST['fecha_fin']."' ";
	...
	$pdf->Cell(50,5,'Beneficiario',1,0,'L');
	...
	while ($row=mysql_fetch_array($res)){	
		...
		$pdf->Cell(50,5,$array_beneficiarios[$row['beneficiario']],1,0,'L');
...
if($_POST['ajax']==1){
	...
	/*$select= " SELECT a.* FROM recibos_salidas as a  WHERE a.".$_POST['ffecha'].">='".$_POST['fecha_ini']."' AND a.".$_POST['ffecha']."<='".$_POST['fecha_fin']."'
				and hora between '".$_POST['hora_ini']."' and '".$_POST['hora_fin']."'";*/
	$select= " SELECT a.* FROM recibos_salidas as a  WHERE {$filtrofechaini} AND {$filtrofechafin}";
	if(nivelUsuario()<=2){
		$_POST['usu']=$_POST['cveusuario'];
	}
//	if ($_POST['hora_ini']>"00:00:00" and $_POST['hora_fin']>"00:00:00"){$select.=" and a.hora between '".$_POST['hora_ini']."' and '".$_POST['hora_fin']."'";}
	...
	if(mysql_num_rows($res)>0) {
		...
		echo '<th>Folio</th><th>Fecha</th><th>Motivo</th><th>Beneficiario</th><th>Unidad</th>
		<th>Monto</th>
		<th>Concepto</th>
		<th>Usuario<br>';
		...
		$res1=mysql_db_query($base,"SELECT a.usuario FROM recibos_salidas as a WHERE 1 $filtro GROUP BY a.usuario ORDER BY a.usuario");
		...
		while ($row=mysql_fetch_array($res)){	
			...
			if($row['estatus']=='C'){
				...
				echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'recibos_salidas.php\',\'_blank\',\'201\','.$row['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$row['cve'].'"></a>';
				if(nivelUsuario()>2)
			...
			echo '<td align="left">'.$array_beneficiarios[$row['beneficiario']].'</td>';
...
if($_POST['ajax']==2){
	if($_POST['clavecancelacion'] == $clavecancelacion){
		mysql_db_query($base,"UPDATE recibos_salidas SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['usuario']."',obscan='".$_POST['obs']."' WHERE cve='".$_POST['id']."'") or die(mysql_error());
	}
	else{
		echo '1';
	}
...
if($_POST['cmd']==2){
	mysql_db_query($base,"INSERT recibos_salidas SET fecha='".fechaLocal()."',hora='".horaLocal()."',
	motivo='".$_POST['motivo']."',beneficiario='".$_POST['beneficiario']."',monto='".$_POST['monto']."',estatus='A',usuario='".$_POST['cveusuario']."',
...
if($_POST['cmd']==1){
	...
	if(nivelUsuario()>1){
		...
		if(document.forma.motivo.value==\'0\'){
			...
			atcr(\'recibos_salidas.php\',\'\',2,\'0\');
		}
		"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
	}
	echo '<td><a href="#" onclick="$(\'#panel\').show();atcr(\'recibos_salidas.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>';
	echo '</tr></table>';
	echo '<br>';
	echo '<table>';
	...
	echo '</select></td></tr>';
	/*echo '<tr><td>Beneficiario</td><td colspan="2"><select name="beneficiario" id="beneficiario"><option value="0">Seleccione</option>';
	foreach($array_beneficiarios as $k=>$v){
		echo '<option value="'.$k.'"';
		echo '>'.$v.'</option>';
	}
	echo '</select></td></tr>';*/
	echo '<tr><td>Beneficiario</td><td colspan="2"><input type="text" class="textField" id="nombeneficiario" value="" size="100"><input type="hidden" name="beneficiario" id="beneficiario" value=""></td></tr>';
	...
	echo '<script>
			...
			$( "#nombeneficiario" ).autocomplete({
		      source: "recibos_salidas.php?ajax=3",
		      minLength: 2,
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onclick="atcr(\'recibos_salidas.php\',\'\',1,0);"><img src="images/nuevo.gif" border="0">&nbsp;&nbsp;Nuevo</a>&nbsp;&nbsp;</td>';
		echo '
				<!--<td><a href="#" onClick="atcr(\'recibos_salidas.php\',\'_blank\',\'200\',\'\')"><img src="images/b_print.png" border="0" title="Imprimir">&nbsp;Imprimir</a></td>-->
			  </tr>';
		echo '</table>';
		echo '<table>';
		...
		echo '</select></td></tr>';
		echo '<tr><td>Beneficiario</td><td colspan="2"><select name="beneficiario" id="beneficiario"><option value="all">Todos</option>';
		foreach($array_beneficiarios as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
...
<Script language="javascript">
	...
	function cancelarSalida(salida){
		if(cancelarRegistro(salida, \'recibos_salidas.php\', 2)) buscarRegistros(0,1);
