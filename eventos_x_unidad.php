...
if($_POST['plazausuario'] == 1)
	...
	$rsMotivo=mysql_query("SELECT * FROM rutas WHERE plaza IN (0,1) ORDER BY nombre");//$rsMotivo=mysql_query("SELECT * FROM rutas WHERE plaza IN (0, ".$_POST['plazausuario'].") ORDER BY nombre");
while($Motivo=mysql_fetch_array($rsMotivo)){
	$array_rutasgps[$Motivo['base']][$Motivo['cvebase']]=$Motivo['nombre'];
}


$array_base=array(1=>'GAMN', 2=>'GPS3');

mysql_select_db('gps');
if($_POST['cmd']==101){
	
	header('Content-type: application/vnd.ms-excel');
header("Content-Disposition: attachment; filename=exceso de Velocidad.xls");
header("Pragma: no-cache");
header("Expires: 0");

		 $reporte='<table width="100%" border="1" cellpadding="4" cellspacing="1" class="" id="tabla1">
		 <tr><td colspan="7" align="center" style="font-size:20px">EXCESOS DE VELOCIDAD</td></tr>
		 <tr><td colspan="7" align="left" style="font-size:15px">Periodo del: '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</td></tr>
		 <tr><td colspan="7" align="right" style="font-size:15px">Fecha: '.fechaLocal().'  '.horaLocal().'</td></tr>
		 <tr><td colspan="7">&nbsp;</td></tr></table>
		 ';
	$reporte .='
			<table width="100%" border="1" cellpadding="4" cellspacing="1" class="" id="tabla1" style="font-size:10px">';
			
	$reporte .='<tr bgcolo="#E9F2F8">';
	$reporte .= '<th>Dispositivo</th>
		<th>Fecha y Hora de la Maxima</th>
		<th>Maxima</th>
		<th>Fecha y Hora de la Minima</th>
		<th>Minima</th>
		<th>Coordenadas de la Maxima</th>
		<th>Eventos</th></tr>';
	...
	while ($dato = mysql_fetch_assoc($consulta)){
		...
		$reporte .= '<td align="center">'.$row1['latitud'].', '.$row1['longitud'].'</td>';
		$reporte .='<td align="center">'.$dato['eventos'].'</td>';

		$reporte .= '</tr>';
		$i++;

	}
	$reporte .= '<tr bgcolo="#E9F2F8"><td colspan="7">'.$i.' Registro(s)</td></tr>';
	$reporte .= '</table>';
/*	require_once("taller/phplot/phplot.php");
	...
	$plot->SetOutputFile("taller/fotos_graf/grafica1.jpg");
	...
	$plot->DrawGraph();
	$reporte .= '<br><img src="taller/fotos_graf/grafica1.jpg?'.date("Y-m-d H:i:s").'">';*/
	echo $reporte;
...
$filtro .= " and a.tipo='deviceOverspeed'";
	...
	echo '</table>';
 	...
 	if ($_POST['cmd']<1) {
		...
		//Busqueda
		echo '<table><input type="hidden" name="plaza" id="plaza" value="1">';//echo '<table><input type="hidden" name="plaza" id="plaza" value="'.$_POST['plazausuario'].'">';
	//	echo''.$select.'';
		echo '<tr>
...
<Script language="javascript">
	function buscarRegistros(){
		...
		if (objeto.readyState != 0) {
			...
			objeto.send("ajax=1&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&device="+document.getElementById("device").value+"&plaza="+document.getElementById("plaza").value+"&tipo="+document.getElementById("tipo").value+"&geocerca="+document.getElementById("geocerca").value+"&ruta="+document.getElementById("ruta").value);
