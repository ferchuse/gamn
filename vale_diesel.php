...
if($_POST['ajax']==1){
	...
	$select= " SELECT a.* FROM vale_diesel as a INNER JOIN parque as b ON (b.cve=a.unidad";
	...
	if ($_POST['gasolinera']!="all") { $select.=" AND a.gasolinera='".$_POST['gasolinera']."'"; }
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		<th><a href="#" onClick="buscarRegistros(1,'.$tipoorden1.')">Unidad</a></th><th>Gasolinera</th>
		<th>Monto</th>
		<th>Usuario<br>';
		...
		$res1=mysql_db_query($base,"SELECT a.usuario FROM vale_diesel as a WHERE 1 $filtro GROUP BY a.usuario ORDER BY a.usuario");
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			if($Abono['estatus']=='C'){
				...
				echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'vale_diesel.php\',\'\',\'201\','.$Abono['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['cve'].'"></a>';
				echo '</td>';
			}
			...
			echo '<td align="left">'.$array_gasolineras[$Abono['gasolinera']].'</td>';
...
if($_POST['cmd']==201){
	$res = mysql_db_query($base,"SELECT * FROM vale_diesel WHERE cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	
	$varimp="Vale de Diesel Nextlalpan|Folio: ".$row['cve']."|";
	$varimp.="Usuario: ".$array_usuario[$row['usuario']].'|';
	$varimp.=$row['fecha']." ".$row['hora']."||";
	$varimp="|Folio Rec: ".$row['abono']."|";
	$varimp.="Unidad: ".$array_unidad[$row['unidad']]."||";
	$varimp.="Gasolinera: ".$array_gasolineras[$row['gasolinera']]."||";
	$varimp.="Monto: $ ".number_format($row['monto'],2)."||El vale tiene una vigencia de 3 dias";
	$varimp.=chr(29)."h".chr(80).chr(29)."H".chr(2).chr(29)."k".chr(2)."12".sprintf("%010s",(intval($row['cve'])))." |";
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr><td>Gasolinera</td><td colspan=""><select name="gasolinera" id="gasolinera"><option value="all">Todos</option>';
		foreach($array_gasolineras as $k=>$v){
