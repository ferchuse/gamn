...
if($_POST['ajax']==1) {
		...
		$select= " SELECT * FROM ".$pre."motivos WHERE 1 ";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			while($row=mysql_fetch_array($res)) {
				rowb();
				echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'motivos.php\',\'\',\'1\','.$row['cve'].')"><img src="images/modificar.gif" border="0" title="Editar '.$row['nombre'].'"></a></td>';
...
if ($_POST['cmd']==1) {
	...
	$select=" SELECT * FROM ".$pre."motivos WHERE cve='".$_POST['reg']."' ";
	$res=mysql_db_query($base,$select);
	...
	echo '
		<tr>';
		if(nivelUsuario()>1)
			echo '<td><a href="#" onClick="if(document.forma.nombre.value==\'\'){alert(\'Necesita introducir el nombre\');}else{atcr(\'motivos.php\',\'\',\'2\',\''.$row['cve'].'\');}"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
	...
	//Formulario 
	echo '<table>';
	echo '<tr><td class="tableEnc">Edicion de motivos</td></tr>';
	echo '</table>';
	$bloqueado='';
	$class='textField';
	echo '<table>';
